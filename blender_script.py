"""
Script for automating the rendering of space debris objects in Blender for YOLO dataset generation.

This script renders space debris objects in a Blender scene and generates images along with YOLO-format annotations
to be used for object detection tasks in machine learning.
...
def get_2d_bounding_box(scene, camera_obj, target_obj):
    ...
    The function projects the object's vertices into camera space, normalizes them based on the camera's view frame,
    and calculates the bounding box in pixel coordinates.
...
def render_dataset(state, state_file, debris_names, total_images, axes, clouds_obj, sun_obj):
    ...
    # Define batch processing limits.
    BATCH_LIMIT = total_images * len(debris_names)  # Images per batch.
    BATCH_NO_LIMIT = 6  # Maximum number of batches to process.

    if current_no_of_batches > BATCH_NO_LIMIT:
        print(f"Batches completed: {current_no_of_batches}, limit is {BATCH_NO_LIMIT}.")
        save_state_and_exit(state, state_file, batch_limit_reached=True)
        return

    # Define dataset splits and corresponding render counts.
    debris_renders_per_split = [
        ('train', int(total_images * 0.8)),  # 80% for training.
        ('val', int(total_images * 0.1)),    # 10% for validation.
        ('test', int(total_images * 0.1))    # 10% for testing.
    ]

    # Set up output directories.
    global_folder = Path("C:/Users/Jai/Downloads/Individual Project Folder/yolov8-project/dataset6")
    output_path = global_folder / f"Debris{current_batch}"
    ...
    for split_name, renders_per_debris in debris_renders_per_split:
        ...
        output_dir_coords = global_folder / "debris_coordinates"
        output_dir_debris_tracking_container_coords = global_folder / "debris_tracking_container_coordinates"
        output_dir_camera_coords = global_folder / "camera_coordinates"
        output_dir_debris_orientation_euler = global_folder / "debris_orientation_euler"
        output_dir_debris_orientation_quaternion = global_folder / "debris_orientation_quaternion"
...
def main():
    ...
    # Set the number of images to render per debris.
    total_images = 50

    # Begin the rendering process.
    render_dataset(state, state_file, debris_names, total_images, axes, clouds_obj, sun_obj)
